...
import posixpath
from distributed_trainning.utils import image_utils
...
class Generator(utils.Sequence):
    ...
    def __getitem__(self, index):
        image = image_utils.load_image(self.file_names[index])
        boxes = image_utils.load_label(self.file_names[index])
        image, boxes = image_utils.resize(image, boxes)
        # image, boxes = util.random_flip(image, boxes)

        image = image[:, :, ::-1].astype(numpy.float32)
        image = image / 255.0
        y_true_1, y_true_2, y_true_3 = image_utils.process_box(boxes)
...
def input_fn(file_names):
    ...
    dataset = tf.data.Dataset.from_generator(
        generator=generator_fn, output_types=output_types, output_shapes=output_shapes
    )
...
class DataLoader:
    def __init__(self):
        super().__init__()
        self.description = {
            "in_image": tf.io.FixedLenFeature([], tf.string),
