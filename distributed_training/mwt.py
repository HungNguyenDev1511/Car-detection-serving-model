...
def test():
    ...
    def test_fn():
        ...
        for file_name in tqdm.tqdm(file_names):
            ...
            image_np, scale, dw, dh = image_utils.resize(image_np)
...
def write_tf_record(queue, sentinel):
    ...
    while True:
        ...
        if file_name == sentinel:
            break
        in_image = image_utils.load_image(file_name)[:, :, ::-1]
        boxes, label = image_utils.load_label(file_name)

        in_image, boxes = image_utils.resize(in_image, boxes)

        y_true_1, y_true_2, y_true_3 = image_utils.process_box(boxes, label)
...
class AnchorGenerator:
    ...
    def get_boxes():
        ...
        for file_name in file_names:
            for box in image_utils.load_label(file_name)[0]:
